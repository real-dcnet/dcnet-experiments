...
output_dir="tcp_ovs2_trs_output"
...
do
	...
	qperf > /dev/null 2>&1 &
	ps -aef | grep qperf | grep -v grep

	total=0
	avg1=0
	for i in `seq 1 5`
	do
		result_line=`ssh nebula111 "/home/rajas/bin/qperf ${rewriting_ip} -lca 0 -rca 0 -t 30 -m ${s} tcp_lat" | tail -1`
		echo $result_line >> ${output_dir}/latency_m${s}_r
	...
	for i in `seq 1 5`
	do
		result_line=`ssh nebula111 "/home/rajas/bin/qperf ${tunneling_ip} -lca 0 -rca 0 -t 30 -m ${s} tcp_lat" | tail -1`
		echo $result_line >> ${output_dir}/latency_m${s}_t
